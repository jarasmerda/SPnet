    ...
    public static IEndpointRouteBuilder MapCpqNextCode(this IEndpointRouteBuilder app)
    {
        app.MapGet("/CpqNextCode", async (BomDb db) =>
        {
            try
            {
                ...
                int nextNumber = IssuedCodesTracker.GetNextNumber(maxId);
                string nextCode = $"S{nextNumber:D4}";

                // Zaregistrujeme okamžitě, aby při rychlém opakování nevydal stejné číslo
                IssuedCodesTracker.Register(nextCode);
