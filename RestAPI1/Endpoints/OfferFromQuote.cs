...
namespace RestAPI1.Endpoints
{
    public static class OfferFromQuote
    {
        public static IEndpointRouteBuilder MapOfferFromQuote(this IEndpointRouteBuilder app)
        {
            ...
            {
                ctx.Response.Headers["Access-Control-Allow-Origin"] = "http://SRV-TERMINAL:5127";
            ...
            //   "attr9":"05"
            // }
            app.MapPost("/offer/from-quote", async (BomDb db, HttpContext context, IConfiguration config) =>
            {
                // ✅ CORS hlavička i na reálné odpovědi
                context.Response.Headers["Access-Control-Allow-Origin"] = "http://SRV-TERMINAL:5127";
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        var checkRes = await checkClient.GetAsync($"http://SRV-TERMINAL:5005/check-offer?code={Uri.EscapeDataString(newCode)}");
